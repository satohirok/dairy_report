...
## 8章:論理設計のグレーノウハウ
* 主キーが役に立たない時の対応
  * 代理キー:入力されているデータに最初から存在しているキーに新たに追加するキー。
    * 代理キーは論理的に不要なキーのため、論理モデルを分かりづらくしてしまうという観点から、極力自然キーを使用した方が良い。
    * 代理キーを使用する場合は,``一意性``,``連続性``を保証することが必要になってくる。
  * 自然キー
    * タイムスタンプ:時点をキーすることで、データ形式をシンプルにすることができる。全体としてレコード数が多くなってしまうという欠点がある。
    * インターバル:期間をキーにしてデータを管理する。レコード数を少なくできるが、SQLに範囲指定を入れる必要があるので、記述が少し複雑になる。
* 列持ちテーブル:配列型を使わずに、配列を模倣する論理設計のこと。
  * メリット:シンプルな設計、入出力のフォーマットと合わせやすい。
  * デメリット:一度作成すると構成を変更することが難しい。無用のNULLを使う必要がある。
  * 列持ちテーブルは特殊な状況でない限り使用しないようにする。
* 多段ビュー:ビューからさらにSELECT文を用いてビューを作ること
  * ビュー:select文を用いて作られた仮想テーブル(データは保持しない)
  * ビューの階層を深くしてしまうと、使用が複雑になり、管理が困難になってしまう。
* データクレンジング:それまで業務で使用されてきたデータをデータベースに登録できる状態にすること。
  * 一意キーの特定:整理した項目の中から一意キーを決める。
  * 名寄せ:似通った名前を寄せ集めて統合する（重複排除、標準化）
